...
def get_state(entity_id: str) -> str:
    response = requests.get(
        f"{HASS_URL}/states/{entity_id}", headers=HEADERS, timeout=10
    )
...
def call_service(service: str, data: dict):
    ...
    response = requests.post(
        f"{HASS_URL}/services/{domain}/{svc}", headers=HEADERS, json=data, timeout=15
    )
    response.raise_for_status()
...
def camera_snapshot(camera_entity: str, path: str):
    call_service("camera.snapshot", {"entity_id": camera_entity, "filename": path})
