...
def test_fixtures_exist():
    ...
    assert os.path.exists(FIXTURES_DIR), f"Fixtures directory not found: {FIXTURES_DIR}"
...
def test_ocr_comparison(ocr_reader, test_image):
    ...
    assert (
        old_result is not None or new_result is not None
...
def test_ocr_comparison_summary(ocr_reader):
        ...
        results.append(
            {
                ...
                "new_conf": new_conf,
                "old_match": (
                    old_result == test_image["expected"]
                    if test_image["expected"]
                    else None
                ),
                "new_match": (
                    new_result == test_image["expected"]
                    if test_image["expected"]
                    else None
                ),
            }
    ...
    print("=" * 100)
    print(
        f"{'Image':<30} {'Expected':<10} {'Old Result':<20} {'New Result':<20} {'Status':<20}"
    )
    print("-" * 100)
    ...
    for r in results:
        ...
        print(
            f"{r['filename']:<30} {r['expected'] or 'N/A':<10} {old_display:<20} {new_display:<20} {status:<20}"
        )
    ...
    if total_with_expected > 0:
        ...
        print(f"\n📊 STATISTICS:")
        print(
            f"   Old algorithm accuracy:  {old_correct}/{total_with_expected} ({old_accuracy:.1f}%)"
        )
        print(
            f"   New pipeline accuracy:   {new_correct}/{total_with_expected} ({new_accuracy:.1f}%)"
        )
...
@pytest.mark.skipif(len(get_test_images()) == 0, reason="No test images available")
def test_new_pipeline_never_worse(ocr_reader):
    ...
    for test_image in get_test_images():
        ...
        if (
            old_result == test_image["expected"]
            and new_result != test_image["expected"]
        ):
            regressions.append(
                {
